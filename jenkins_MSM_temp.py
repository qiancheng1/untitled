...
class Args(object):
    ...
    def args_check(self):
        PROJECT_NAME_LIST=['E300L','E300L_8.1','A306']
        BUILD_PROJECT_LIST=['E300L_WW','E300L_CN','E300L_IN','A306']
...
def down_load_code(url,branch,xml):
    ...
    projects_mirror={('E300L','A306'):'300_mirror_repo',('E300L_8.1',):'300_8.1_mirror_repo'}
...
def process_release_file():
    ...
    if args_obj.MODULE == 'overall':
        #os.remove('overall_all_images.tar.gz.apk')
        shutil.copy('overall_symbols.zip','/data/mine/test/MT6572/jenkins/')
        #if args_obj.BUILD_PROJECT == 'E300L_IN':
        #    shutil.copytree('../out/target/product/E300L_WW/overall_all_images','{0}_DL'.format(args_obj.IN_VERSION))
        #else:
        #    shutil.copytree('../out/target/product/{0}/overall_all_images'.format(args_obj.BUILD_PROJECT),'{0}_DL'.format(args_obj.IN_VERSION))
        #get_fastboot(args_obj.IN_VERSION+'_DL/')
        #make_zip(args_obj.IN_VERSION+'_DL',args_obj.IN_VERSION+'_DL.zip')
        #overall  的时候什么都不要做。版本会自动释放出来的。然后cp出去
    else:
