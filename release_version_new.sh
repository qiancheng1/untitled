...
function change_sign(){
    ...
    for tmp in $RELEASE_FILES           #就是要释放的各个版本
    do
        change_flag=0
        ...
        do
            sign_tmp_middle=$(echo $sign_tmp | awk -F "." '{print $1}')  #各个img的名字
            if [ $flag_6750 == "true" ]
            then
...
function parse_args(){
    ...
    readonly build_param="$1"     #build_param是什么？ --->就是编译释放的项目名
    readonly args_2="$2"
    if [ x"$build_param" = "x" ]
    then
    ...
    [ x"$2" == x"symbols" ] && readonly symbols_flag=1 && release_param="all"  #不是很清楚这要表达什么意思 ./read_version.sh E281L all sign
    [ x"$3" == x"sign" ] && sign_flag="yes"
    [ x"$2" == x"sign" ] && sign_flag="yes" && release_param="all"
    [ "$release_param" != "all" ] && release_param="$args_2"
    [[ "$all_args" =~ "zip" ]] && readonly ZIP_VERSION="true"  #这个是在哪里？ --->在所有传递进来的参数里面 =~正则匹配
    [ x"$release_param" = "x" ] &&  release_param=all     #默认为all
...
function init_config(){
    ...
    case $build_param in
        ...
        A460)
           HARDWARE_VER=S01
           OUT_PATH=$OUT_PATH/A460
           readonly PLATFORM=MT6739
           ;;
        *)
    ...
    if [ -s "$OUT_PATH/system/build.prop" ];then
        ...
        local -r read_version=$(grep -n "ro.product.hardware=*" $BUILD_FILE | cut -d "=" -f 2)  # null
...
function all_system_file_ready(){
    ...
    if [ x"$release_param" = x"all" ]; then
        ...
        elif [ $build_param == A460 ];then
            for i in "$OUT_PATH/obj/ETC/MDDB_InfoCustomAppSrcP_MT6739_S00_MOLY_LR12A_*_1_ulwtg_n.EDB_intermediates/MDDB_InfoCustomAppSrcP_MT6739_S00_MOLY_LR12A_*_1_ulwtg_n.EDB" ; do
                if [ -f $i ]; then
                    cp $i  $OUT_PATH/Modem_Database_ulwtg
                fi
            done
        elif [ $flag_6750 == "true" ];then
            for i in "$OUT_PATH/obj/ETC/MDDB_InfoCustomAppSrcP_MT6739_S00_MOLY_LR12A_*_1_ulwctg_n.EDB_intermediates/MDDB_InfoCustomAppSrcP_MT6739_S00_MOLY_LR12A_*_1_ulwctg_n.EDB" ; do
                if [ -f $i ]; then
	...
	then
            ...
            for i in "$OUT_PATH/obj/CGEN/APDB_MT6739_"$HARDWARE_VER"_alps-trunk-o1.bsp_W??.??" ; do
                if [ -f $i ]; then
                    cp $i $OUT_PATH/AP_Database
                fi
            done
    ...
    if [ $build_param == ZB500TLVW ];then
        ...
        if [ ! -f $OUT_PATH/Modem_Database_ulwctg ];then
            ALL_RELEASE_FILES_EXCEPT_SYSTEM="logo.bin $PLATFORM"_Android_scatter.txt"
            preloader_${build_param}.bin AP_Database Modem_Database_ulwtg boot.img secro.img
            userdata.img lk.img recovery.img cache.img mcupmfw.img md1dsp.img loader_ext.img spmfw.img tee.img md1img.img odmdtbo.img vendor.img"
        elif [ ! -f $OUT_PATH/Modem_Database_ulwtg ];then
            ALL_RELEASE_FILES_EXCEPT_SYSTEM="logo.bin $PLATFORM"_Android_scatter.txt" preloader_${build_param}.bin AP_Database Modem_Database_ulwctg boot.img secro.img userdata.img lk.img recovery.img cache.img mcupmfw.img md1dsp.img loader_ext.img spmfw.img tee.img md1img.img odmdtbo.img vendor.img"
        else
            ALL_RELEASE_FILES_EXCEPT_SYSTEM="logo.bin $PLATFORM"_Android_scatter.txt" preloader_${build_param}.bin AP_Database Modem_Database_ulwctg Modem_Database_ulwtg boot.img secro.img userdata.img lk.img recovery.img cache.img mcupmfw.img md1dsp.img loader_ext.img spmfw.img tee.img md1img.img odmdtbo.img vendor.img"
        fi
        SIGN_RELEASE_FILES="APD-sign.img boot-sign.img cache-sign.img lk-sign.bin logo-sign.bin recovery-sign.img secro-sign.img system-sign.img trustzone-sign.bin userdata-sign.img boot_su-sign.img"
    fi
...
function cp_release_file_to_tmp(){
    ...
    pushd "$ROOT" > /dev/null
        ...
        tmp_dir=$(mktemp -d back_up_version_temp_XXXX)   # xxxx后面是一个四位随机数，创建目录
    ...
    case $rename_type in
        ota) 
            ...
            pushd "${tmp_dir}" > /dev/null
                ...
                local -r sku=$(sed -n "s/ro.build.asus.sku=//g"p "$BUILD_FILE" | head -1)   #null
        ...
        case $local_release_param in
            all) 
                ...
                then
                    ...
                    local -r Plat_version_name="$(sed -n  "s/ro.build.version.plan.inner=//g"p "$OUT_PATH/system/build.prop")_DL"
                    local Outer_version_name=$(sed -n  "s/ro.build.display.id=//g"p "$OUT_PATH/system/build.prop")
...
function release_files(){
    ...
    process_release_file "$local_release_param" "$FILES"
    if [ "$local_release_param" != "diff" ]
    then
...
then
    ...
    readonly last_release_param=$(echo $final_release_param | awk '{print $NF}')    #awk '{print NF}' 会输出每列的行数 $NF是最后一个字段的内容
